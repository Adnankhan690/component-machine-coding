...
export default function MultipleFileUploader() {
	...
	const handleFileUpload = async () => {
		...
		if (uploadStrategy === "stopOnError") {
			// Case 1: Stop uploading if any file fails (Promise.all)
			await handleFileUploadStopOnError();
		} else {
			// Case 2: Continue uploading even if some files fail (Promise.allSettled)
			await handleFileUploadContinueOnError();
		}
	};

	// Case 1: Stop on first error - uses Promise.all()
	const handleFileUploadStopOnError = async () => {
		try {
			...
			// Wait for ALL to succeed, or fail on first error
			await Promise.all(uploadPromises);

			// If we reach here, ALL uploads succeeded
			setFiles((prevFiles) => {
			...
			// If ANY upload fails, mark ALL remaining files as error
	...
	// Case 2: Continue even on errors - uses Promise.allSettled()
	const handleFileUploadContinueOnError = async () => {
		const uploadPromises = files.map(async (file) => {
			try {
				const formdata = new FormData();
				formdata.append("file", file.file);
			...
			<div style={{ margin: "10px 0" }}>
				<label>
					<input
						...
						checked={uploadStrategy === "continueOnError"}
						onChange={(e) =>
							setUploadStrategy(e.target.value as "continueOnError")
						}
					/>
				...
				<label>
					<input
						...
						name="uploadStrategy"
						value="stopOnError"
						checked={uploadStrategy === "stopOnError"}
						onChange={(e) => setUploadStrategy(e.target.value as "stopOnError")}
					/>
					Stop uploading if any file fails
				</label>
			</div>
			<button onClick={handleFileUpload}>
				Upload (
				{uploadStrategy === "stopOnError"
					? "Stop on Error"
					: "Continue on Error"}
				)
			</button>
